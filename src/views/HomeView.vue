...
// ---- Component Imports ------
import aboutMeDialog from "@/components/aboutMeDialog.vue"
...
const isCollision = (nextX, nextY, w, h, currentId) => {
...
<template>
  ...
  <div ref="vantaRef" class="relative w-screen h-screen">
    ...
    <div class="relative z-10 text-white text-center h-full center-all">
      ...
      <aboutMeDialog v-model:visible="showAbout" />
      <experienceDialog v-model:visible="showExperience" />
      <!-- =========== DIALOGS SECTION ( END ) =========== -->

      <!-- =========== GAME AREA MAIN CONTAINER =========== -->
      <main class="flex flex-col center-all w-full h-full">

        <!-- NAVBAR -->
        <div
          class="flex items-center w-[69rem] h-[4rem] mb-2 bg-white/10 backdrop-blur-md border border-white/60 shadow-md">
          ...
          <div class="bg-slate-500 ml-auto mr-4 px-4 py-2 text-white rounded cursor-pointer">Mode</div>
        ...
        <div :class="`relative w-[${containerWidthInRem}rem]`">
          ...
          <div v-for="block in blocks" :key="block.id" @click="selectBlockToMove(block), toggleContent(block)" :class="[
            'absolute text-white text-[1.5rem] flex flex-col justify-end cursor-pointer border backdrop-blur-md transition duration-300 group',
            block.id === 'A' ? 'bg-black/50 hover:bg-black/70 border-white/60' : 'bg-black/30 hover:bg-black/50 border-white/40'
          ]" :style="{
            ...
            <div v-if="block.id !== 'A'" class="w-full h-[5rem] flex items-end flex-col">
              <p
                class="text-[0.8rem] text-[#77FFA4] mr-5 mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                ← Tap to view content
              </p>
              <p class="mr-5 font-semibold">
                {{ block.labelName }} <span class="text-[#77FFA4]">;</span>
              </p>
            </div>
            ...
            <div v-if="block.id === 'A'" class="w-full h-full center-all flex flex-col ">
              ...
              <p class="text-[1rem] text-[#77FFA4] items mt-2 opacity-0 group-hover:opacity-100"> - Tap to view content
                - </p>
            </div>
